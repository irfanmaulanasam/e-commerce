  ...
  <div class="">
    <v-container grid-list-md text-xs>
      ...
      <div v-if="page == 'pendingtransactions'">
        <v-card v-for="(transaction, index) in pendingTrans" :key="index">
          <v-container
            fluid
            grid-list-lg
          >
          <v-layout row wrap>
            <v-flex xs4>
              <h3 v-for="(cart, index) in transaction.carts" :key="index">{{cart.item.name}} X {{cart.item.quantity}}</h3><br>
      ...
      <div v-if="page == 'onprocesstransactions'">
        <v-card v-for="(transaction, index) in onprocessTrans" :key="index">
          <v-container
            fluid
            grid-list-lg
          >
      ...
      <div v-if="page == 'transactionsdone'">
        <v-card v-for="(transaction, index) in doneTrans" :key="index">
          <v-container
            fluid
            grid-list-lg
          >
  ...
  methods: {
    ...
    deleteTransaction (id) {
      axios.delete(`${this.url}/transactions/${id}`)
        .then((response) => {
          this.getTransactions()
          toastr.info('product have been added')
        })
        .catch((error) => {
      ...
      axios.post(`${this.url}/users/admincheck`, { token: localStorage.getItem('token') })
        .then((response) => {
          if (response.data.admin == false) {
            ...
            toastr.warning('you are not admin')
