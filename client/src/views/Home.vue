        ...
        <v-btn color="success">buy</v-btn>
            <v-dialog v-model="dialog" width="600px">
              <template v-slot:activator="{ on }">
                <v-btn color="info" dark v-on="on">detail</v-btn>
              </template>
              <v-card>
                <v-img src="https://picsum.photos/510/300?random" aspect-ratio="1.7"/>
                <v-card-title>
                  <span class="headline">Ini untuk bagian nama kopi</span>
                </v-card-title>
                ...
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="success" @click="dialog = false" >Add to Cart</v-btn>
                  <v-btn color="warning" @click="dialog = false">cancel</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </v-layout>
        <v-btn color="error" v-if="isAdmin">delete</v-btn>
        </v-img>
      </v-flex>
    </v-layout>
  </v-container>
